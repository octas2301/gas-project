...
- **フォーカス領域**: **AI組織 Phase0** — [org/AI_ORG_CHARTER.md](org/AI_ORG_CHARTER.md) と [org/AI_APPROVAL_MATRIX.md](org/AI_APPROVAL_MATRIX.md) を正本化し、**3者レビュー＋多数決**で実現性・要件漏れを潰す。**この Phase0 では実装コードを書かない。**
- **このフェーズの完了条件（目安）**  
  - 憲章・承認マトリクスが docs にあり、人間が目視で矛盾なしと判断した。  
  - 3者検証（別モデル独立レビュー→2/3多数決）を反映し、未決事項が洗い出されている。  
  - 次フェーズ（Lv別プラン／在庫0日中出品の実装）への着手条件が文書上はっきりしている。

- **並行・継続（後回し可）**: 商品情報まわりの OpenAI 429 切り分け、Z「11-③」と B Step7 の差の検証（[AI_ROUTING_GEMINI_OPENAI.md](AI_ROUTING_GEMINI_OPENAI.md)）。
...
- **AI組織**: 朝モバイル承認 → 日中無人出品（原則12:00前） → 夜確認。在庫0原則（不可なら1）。補充は別承認で `FLOOR(仕入÷セット)`。CS／問屋は送信禁止・下書き可。承認UI既定は GAS Web（Googleログイン）、LINE可。詳細は org 憲章・マトリクス。  
...
1. **組織憲章・承認マトリクスの目視確認**のうえ、**3者レビュー＋多数決**（手順は憲章 §9）。  
2. 多数決反映後、**Lv別プラン**（在庫0日中出品・Amazonバルク等）を策定し実装フェーズへ。  
...
| テーマ | ドキュメント |
|--------|----------------|
| **AI組織・承認** | [org/AI_ORG_CHARTER.md](org/AI_ORG_CHARTER.md)、[org/AI_APPROVAL_MATRIX.md](org/AI_APPROVAL_MATRIX.md) |
