...
- **Track B 親行の inventory は常に0**（2026-08-05）。`inventoryMode=MASTER` でも親マスタ「在庫数」は読まない（実在庫・厳密チェックは子のみ）。親の `#DIV/0!` 等で送信停止しない
- **販売可能数への引き上げはしない**（承認②）。ただし **D×P0** で人間が「マスタ在庫で出す」を選び専用トグルONのときのみ、バルク／Listings の送信数量にマスタ由来値を載せてよい（マスタ列非書込・承認②相当）。正本: [LV4_D_P0_E_ABSORB_INVENTORY_APPROVAL.md](LV4_D_P0_E_ABSORB_INVENTORY_APPROVAL.md)
- **販売中SKUの無人上書き（U1）は当面手動・後送り**（Q11b=A）。既に Amazon で売れている（マスタ在庫>0 等でスキップ対象）SKUへの更新バルクは Lv4 では出さない。`inventoryMode` は **新規／再掲載用バルクに載せる在庫値**の話に限る（P0マスタqtyは人間明示選択時のみ）
