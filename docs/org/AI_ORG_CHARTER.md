...
1. **当面は不可逆操作をすべて承認制**とする（自動送信・無承認の在庫付き販売開始を禁止）。
2. **お客様対応・メーカー／問屋対応の自動送信は禁止**。下書き生成・受信の振分／要約／緊急フラグは許可。
3. **出品は在庫0を原則**（買えない状態で掲載）。モール仕様または検証で不可なら **在庫1をフォールバック**。いずれでも **販売可能数への引き上げ（補充）は別承認**。
4. 補充数は **`FLOOR(仕入数 ÷ セット数)`**（切捨て。余りは在庫に載せない）。
5. **失敗時に出品データを消さない**。在庫を付けない／補充しないで止める。
6. **検収の「100点」は人が書いたチェックリスト通過**を意味する。AI の自己採点は不合格扱い。
7. **楽天既存ロジック聖域・Yahoo 分離・B統合 Step 境界**は [AGENTS.md](../../AGENTS.md) を継承する。
...
- [ ] 本憲章と [AI_APPROVAL_MATRIX.md](AI_APPROVAL_MATRIX.md) が docs にあり、人間が目視で矛盾なしと判断した  
- [ ] **3者レビュー＋多数決**で実現性・要件漏れ・矛盾を検証し、必要なら正本を更新した  
- [ ] **実装コードはまだ書かない**（次フェーズ）

### 次アクション（3者検証の手順要旨）

1. 別モデルの Cloud／Agent を最大3本、**独立**に次を採点させる（新規構想の勝手な拡大は禁止）。  
   - 入力: 本ファイル＋承認マトリクス＋ [AGENTS.md](../../AGENTS.md) §2/§5/§8  
   - 観点: 実現性（12時前×3モール）、要件漏れ、矛盾、聖域、要実機の逃げ漏れ  
2. 4本目またはローカル Agent で **2/3以上採用・1票は未決事項** の多数決 MD を書く。  
3. 社長が最終承認後、Lv別プラン（実装順序）へ。
