...
**バージョン表示の更新（反映確認用）**  
push／デプロイ前に、リポジトリルートで以下を実行するとバージョンが現在日時に更新されます。

- **外部スキャンページ**: `SCAN_PAGE_VERSION` を更新  
  `.\scripts\update-scan-version.ps1`
- **在庫アプリ（GAS）**: 画面上の「v 日時」とデプロイ一致確認用に `INVENTORY_APP_VERSION` を更新  
  `.\scripts\update-inventory-app-version.ps1`

その後、commit → push（および GAS の場合は clasp push → 新バージョンでデプロイ）してください。在庫アプリは画面上部の「表示: ○○ | base: あり | v 日時」でどれがデプロイされているか確認できます。

**postMessage 時のログ（デバッグログシート）**
- `[在庫アプリ] externalScanResult view=○ showView_called inputId=○` … 受信し showView を実行した（新コードが動いている）
- `[在庫アプリ] externalScanResult view_transition_mismatch expected=○ actual=△` … 画面遷移が期待と異なった（不具合の切り分け用）
